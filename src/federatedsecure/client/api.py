...
class Api:
    ...
    def __init__(self, url=None, interface=None):
        """connects to the Federated Secure Computin API at url or through some specific interface"""
        if url is not None:
    ...
    def download(self, representation):
        """serializes the object represented by uuid, and returns the serialized data"""
    ...
    def attribute(self, representation_uuid, member_name):
        """gets attribute (e.g., child variable, member function) of object represented by uuid,
        stores the pointer on the server side, and returns uuid representing the attribute"""
